...
import { validarPayloadActivo } from '../../../lib/validacion-activos';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const payload = validarPayloadActivo(body);
    const supabase = crearClienteSupabaseDeRequest(request);
    ...
    if (error) {
      if (error.code === '42501') {
        throw new ErrorApi(403, 'No tienes permiso para crear activos en este espacio');
      }
...
function manejarError(err: unknown) {
  ...
  console.error('Error inesperado en /api/activos:', err);
